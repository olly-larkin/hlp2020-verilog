...
let waveStateChangeFunctionTests = 
    ...
    let testWaveformState:WaveformState = {prevVal=0UL; svgVals = Polyline([0.0, 3.0], styleprops, None)}
    let incorrectWaveformState:WaveformState = {prevVal= 0UL; svgVals=Group([],[],None)}
    testList "Wire State Change Tests for indv. state change functions" ([
        ([
            "Low2Low Functional 0 to 0 test",
                testWaveformState |> Low2Low,
                    {prevVal = 0UL; svgVals = Polyline([0.0,3.0; 5.0,3.0], styleprops,None)}
            
            "Low2High Functional 0 to 1 test",
                testWaveformState |> Low2High,
                    {prevVal = 1UL; svgVals = Polyline([0.0,3.0; 0.0,0.0;5.0,0.0], styleprops,None)}

            "High2Low Functional 1 to 0 test",
                testWaveformState |> High2Low,
                    {prevVal = 0UL; svgVals = Polyline([0.0,3.0;0.0,3.0;5.0,3.0], styleprops,None)}

            "High2High Functional 1 to 1 test",
                testWaveformState |> High2High,
                    {prevVal = 1UL; svgVals = Polyline([0.0,3.0;5.0,0.0], styleprops,None)}
...
let waveStateChangeMatchTests = 
    let initLowTestWaveformState:WaveformState = {prevVal=0UL; svgVals = Polyline([0.0, 3.0], styleprops, None)}
    let initHighTestWaveformState:WaveformState = {prevVal=1UL; svgVals = Polyline([0.0, 3.0], styleprops, None)}
    let incorrectWaveformState:WaveformState = {prevVal= 2UL; svgVals=Group([],[],None)}
    testList "Wire State Change Tests for match function" ([
        ([
            "Low2Low Call test",
                (initLowTestWaveformState,0UL) ||> SingleWireCycle,
                    {prevVal = 0UL; svgVals = Polyline([0.0,3.0; 5.0,3.0], styleprops,None)}                 

            "Low2High Call test",
                (initLowTestWaveformState,1UL) ||> SingleWireCycle,
                    {prevVal = 1UL; svgVals = Polyline([0.0,3.0; 0.0,0.0;5.0,0.0], styleprops,None)}
                 
            "High2Low Call test",
                (initHighTestWaveformState,0UL) ||> SingleWireCycle,
                    {prevVal = 0UL; svgVals = Polyline([0.0,3.0;0.0,3.0;5.0,3.0], styleprops,None)} 

            "High2High Call test",
                (initHighTestWaveformState,1UL) ||> SingleWireCycle,
                    {prevVal = 1UL; svgVals = Polyline([0.0,3.0;5.0,0.0], styleprops,None)} 

            ] |> List.map equalTestAsync)
        ([
            "state.prevVal:Incorrect, cycle:Correct Error test",
                (fun _ -> SingleWireCycle incorrectWaveformState 0UL |> ignore),
                    "Input value not valid"
             
            "state.prevVal:Correct, cycle:Incorrect Error test",
                (fun _ -> SingleWireCycle initLowTestWaveformState 2UL |> ignore),
                    "Input value not valid"
            
            "state.prevVal:Incorrect, cycle:Incorrect Error test",
                (fun _ -> SingleWireCycle incorrectWaveformState 2UL |> ignore),
                    "Input value not valid"
...
let busStateChangeFunctionTests = 
    ...
    let testBusState1:WaveformState = {prevVal=0UL; svgVals = Group([Polyline([0.0, 3.0], [], None)], styleprops, None)}
    let testBusState2:WaveformState = {prevVal=5UL; svgVals = Group([Polyline([0.0, 3.0], [], None)], styleprops, None)}
    let test1ExpOutput:WaveformState = {prevVal=0UL; svgVals = Group([Polyline([0.0, 3.0], [], None); Polyline([0.0, 0.0;5.0, 0.0], [], None);Polyline([0.0, 3.0;5.0, 3.0], [], None)], styleprops, None)}
    let test2ExpOutput:WaveformState = {prevVal=5UL; svgVals = Group([Polyline([0.0, 3.0], [], None); Polyline([0.0, 0.0;5.0, 0.0], [], None);Polyline([0.0, 3.0;5.0, 3.0], [], None)], styleprops, None)}
    let test3ExpOutput:WaveformState = {prevVal=5UL; svgVals = Group([Polyline([0.0, 3.0], [], None); Polyline([0.0, 0.0;0.5, 3.0; 5.0,3.0], [], None);Polyline([0.0, 3.0;0.5, 0.0;5.0, 0.0], [], None); Text((0.6,1.9),"0x5",[("fill", "black")],None)], styleprops, None)}
    let test4ExpOutput:WaveformState = {prevVal=0UL; svgVals = Group([Polyline([0.0, 3.0], [], None); Polyline([0.0, 0.0;0.5, 3.0; 5.0,3.0], [], None);Polyline([0.0, 3.0;0.5, 0.0;5.0, 0.0], [], None); Text((0.6,1.9),"0x0",[("fill", "black")],None)], styleprops, None)}
    let incorrectBusWaveformState:WaveformState = {prevVal=0UL; svgVals = Polyline([0.0, 3.0], styleprops, None)}
    testList "Bus State Change Tests for indv. state change functions" ([
        ([
            "busSameVal Functional 0 to 0 test",
                (testBusState1, 0UL) ||> busSameVal,
                    test1ExpOutput
            
            "busSameVal Functional 5 to 5 test",
                (testBusState2, 5UL) ||> busSameVal,
                    test2ExpOutput

            "busChangeVal Functional 0 to 5 test",
                (testBusState1,5UL) ||> busChangeVal,
                    test3ExpOutput

            "busChangeVal Functional 5 to 0 test",
                (testBusState2,0UL) ||> busChangeVal,
                    test4ExpOutput
                 
            ] |> List.map equalTestAsync)
        ([
            "busSameVal Error test",
                (fun _ -> busSameVal incorrectBusWaveformState 0UL |> ignore),
                    "Error constructing bus waveform"
             
            "busChangeVal Error test",
               (fun _ -> busChangeVal incorrectBusWaveformState 0UL |> ignore),
                    "Error constructing wave"
    ...
    testList "Bus State Change Tests for match function" ([
        ([
            "busSameVal Call test1",
                (testBusState1,0UL) ||> SingleBusCycle,
                    test1ExpOutput                 

            "busSameVal Call test2",
                (testBusState2,5UL) ||> SingleBusCycle,
                    test2ExpOutput
                 
            "busChangeVal Call test1",
                (testBusState1,5UL) ||> SingleBusCycle,
                    test3ExpOutput 

            "busChangeVal Call test2",
                (testBusState2,0UL) ||> SingleBusCycle,
                    test4ExpOutput
...
let helperFunctionTests = 
    ...
    let (testMap:Map<int, WireVal list>) = Map[(1,[0UL;1UL])]
    let textBoxFuncTest =
    ...
    let addPinValToMapTest1 =
        equalTestAsync ("Function: addPinValToMap, Input:Map, int*int, Output:Map Test:Add element to pre-existing key", (addPinValToMap testMap (1,1UL)), Map[(1,[0UL;1UL;1UL])])

    let addPinValToMapTest2 =
        equalTestAsync ("Function: addPinValToMap, Input:(Map, int*int), Output:Map Test:Add element to key not in map", (addPinValToMap testMap (2,1UL)), Map[(1,[0UL;1UL]);(2,[1UL])])

    let keyCheckTest1 =
        equalTestAsync ("Function: KeyCheck, Input:(Map, int), Output:int list, Test: Check for key already in map", (keyCheck testMap 1), [0UL;1UL])
